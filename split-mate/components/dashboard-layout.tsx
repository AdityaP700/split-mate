...
// --- CHANGE 1: Import our new MyNetwork component ---
import MyNetwork from "@/components/sections/my-network"; 
// Note: You will need to create placeholder files for Button and Input if you don't have them from shadcn
// e.g., in /components/ui/button.tsx and /components/ui/input.tsx
  ...
  return (
    <div className="min-h-screen bg-[#030815] text-white font-sans">
      <div className="fixed left-0 top-0 h-full w-64 bg-[#080d1b]/50 backdrop-blur-lg border-r border-white/5">
        <div className="p-6 flex flex-col h-full">
          ...
          <nav className="space-y-2 flex-grow">
            {TABS.map((item, idx) => (
              <button key={item.label} onClick={() => setActiveTab(idx)} className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg text-left transition-all duration-200 relative ${ activeTab === idx ? "text-white bg-white/5" : "text-white/60 hover:text-white hover:bg-white/5" }`}>
                {activeTab === idx && (<motion.div layoutId="active-tab-indicator" className="absolute left-0 top-0 bottom-0 w-1 rounded-r-full" style={{background: brandColor}} />)}
          ...
          <div className="mt-auto pt-6 border-t border-white/5">
                <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => disconnect()} className="p-2 rounded-lg text-white/60 hover:bg-red-500/20 hover:text-red-400 transition-colors"><LogOut className="w-5 h-5"/></button>
                </div>
          </div>
        </div>
      </div>
      ...
      <main className="pl-64 p-8">
              ...
              {statCards.map((stat, index) => (
                <Card key={index} className="bg-white/5 border border-white/10 shadow-lg shadow-black/20 rounded-xl">
                  <CardContent className="p-6">
                      <div className="flex items-center justify-between mb-4"><p className="text-sm font-medium text-white/60">{stat.title}</p><stat.icon className={`w-5 h-5 ${stat.positive ? "text-green-400" : "text-red-400"}`} /></div>
                      <div className="flex items-baseline gap-2"><p className="text-3xl font-bold">{stat.amount}</p><span className={`text-sm font-medium ${stat.positive ? "text-green-400" : "text-red-400"}`}>{stat.change}</span></div>
                  </CardContent>
                </Card>
              ))}
            </div>
        ...
        {/* --- CHANGE 2: Implement the MyNetwork component --- */}
        {activeTab === 2 && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                <MyNetwork />
            </motion.div>
